...
def do_deploy(archive_path):
    ...
    try:
        ...
        run(f"rm /tmp/{basename}.tgz")
        run(f"mv /data/web_static/releases/{basename}/web_static/* "
            f"/data/web_static/releases/{basename}/")
        run(f"rm -rf /data/web_static/releases/{basename}/web_static")
        run(f"rm -rf /data/web_static/current")
        run(f"ln -s /data/web_static/releases/{basename}/ "
...
def do_clean(number=0):
    ...
    end = max(int(number), 1)
    with lcd('./versions'):
        local(f'ls -t | grep -v "test" | sed 1,{end}d | xargs rm -rf')
    with cd("/data/web_static/releases"):
        run(f'ls -t | grep -v "test" | sed 1,{end}d | xargs rm -rf')
